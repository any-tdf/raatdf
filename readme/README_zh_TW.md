...
RAATDF 是一個高效、優雅、輕量級的企業級後台管理模板，採用最新技術棧構建，提供開箱即用的解決方案。

# 快速連結

📺 [線上演示](https://preview.raatdf.com) | 📖 [使用文件](https://raatdf.com) | 🐙 [GitHub 倉庫](https://github.com/any-tdf/raatdf)

# 核心技術棧

- **建構工具**: [Vite](https://vitejs.dev) v8 (Beta) - 極速開發體驗
- **前端框架**: [React](https://react.dev) v19 - 最新版本，支援 React Compiler
- **類型系統**: [TypeScript](https://www.typescriptlang.org) v5 - 嚴格類型檢查
- **UI 框架**: [Ant Design](https://ant.design) v6 - 企業級 UI 元件庫
- **樣式方案**: [Tailwind CSS](https://tailwindcss.com) v4 - 原子化 CSS 框架
- **狀態管理**: [Zustand](https://zustand-demo.pmnd.rs) v5 - 輕量級狀態管理
- **路由方案**: [React Router](https://reactrouter.com) v7 - 最新路由解決方案
- **程式碼品質**: [Biome](https://biomejs.dev) - 統一的程式碼檢查和格式化工具
...
- **浮動 UI**: 優雅的浮動 UI 風格
- **沉浸模式**: 專注內容的全螢幕體驗
- **側欄工具**: 將工具列放置在側邊欄底部（僅垂直佈局）

## 主題系統

- **明暗主題**: 亮色、暗色、跟隨系統三種模式
- **主題色**: 10+ 預設顏色 + 自訂顏色
- **動態切換**: 透過 CSS 變數實現無縫主題切換
- **圓角大小**: 9 種圓角尺寸可選（0-24px）

## 互動增強

- **多標籤頁**: 最多 16 個標籤頁，自動關閉最早標籤
- **頁面快取**: 保留頁面狀態，提升使用者體驗
- **標籤風格**: 預設、按鈕、簡潔、堆疊四種風格
- **過渡動畫**: 滑動、淡入、縮放等多種動畫效果
- **緊湊模式**: 適合資料密集型場景
...
```bash
# 程式碼檢查和格式化
bun run check        # 執行 Biome 檢查
bun run check:fix    # 自動修復程式碼問題
bun run lint         # 僅檢查程式碼規範
bun run lint:fix     # 自動修復程式碼規範問題
bun run format       # 檢查程式碼格式
bun run format:fix   # 自動格式化程式碼
```
...
**建議**: 使用最新版本的現代瀏覽器以獲得最佳體驗。

# 參與貢獻

我們歡迎所有形式的貢獻，無論是新功能、錯誤修復還是文件改進。
...
請遵循 [約定式提交](https://www.conventionalcommits.org/zh-hant/)：
...
- 💬 [GitHub Issues](https://github.com/any-tdf/raatdf/issues) - 錯誤報告和功能請求
- 🔀 [GitHub Discussions](https://github.com/any-tdf/raatdf/discussions) - 討論和問答
...
<a href="https://github.com/any-tdf/raatdf/graphs/contributors">
  <img src="https://contrib.nn.ci/api?repo=any-tdf/raatdf" alt="貢獻者" />
</a>

# 贊助商

如果本專案對您有幫助，請考慮贊助以支援我們的持續開發。

<!-- 贊助商清單 -->

# 開源協議

本專案基於 [MIT](https://github.com/any-tdf/raatdf/blob/main/LICENSE) 協議開源。
